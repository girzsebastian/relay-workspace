...
`desktop/files.cjs` limits editor access to existing files inside a chosen project and compares content hashes before a save. The editor persists up to 20 tabs per repository (2 MB per file, 8 MB combined draft content), two active editor paths, focus, and expanded directories. Both editor groups share each file buffer. Closing dirty tabs requires a native discard dialog; switching files preserves drafts. It does not yet provide new-file creation, refactoring, language servers, or a git diff view.
...
`desktop/agents.cjs` gives each named agent a provider, project, instructions, optional skill, and last-session reference. Tasks launch official CLI conversations through the PTY manager. Exit/interruption moves the agent into review; manual review does not kill a process. Recovery retains agent identity. Agents share project files; worktree isolation and unattended orchestration remain future work.
...
Existing version-1 state is migrated additively: legacy drafts become tabs; absent agent collections are initialized; the old overview opens as the editor. The file format version remains 1 for this additive migration.
