# Relay 0.3.1 — agents and editor color
...
Shared editor buffers synchronize during rendering to prevent an older pending update from overwriting rapid typing.
...
- `Relay-0.3.1-mac-arm64.zip`: Apple Silicon Mac app. Quit Relay after finishing active work, extract the ZIP, and replace Relay in Applications.
- `Relay.Setup.0.3.1.exe`: Windows x64 installer, cross-built on macOS.
- `Relay-0.3.1-source.zip`: source at this release tag.
...
This remains an unsigned desktop alpha. Windows CI passes core tests, desktop interactions, PTY I/O, restart recovery, and native packaging; the downloadable NSIS installer has not been installation-tested. No CLI token collection, unattended scheduling, browser/computer agent tools, VS Code extension host, or hosted subscription service is included. See [validation](VALIDATION.md) and [Rakazo assessment](RAKAZO.md) for details.
